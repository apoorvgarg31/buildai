  ...
  return (
    <div className="flex flex-col h-full bg-[#0a0a0a]">
      ...
      <div className="flex-1 overflow-y-auto px-6 py-5">
        {loading ? (
          <div className="flex items-center justify-center h-32 text-gray-500 text-sm">Loading connections...</div>
        ) : connections.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-32 text-gray-500 text-sm">
            <p>No connections yet</p>
            <button onClick={() => setShowAddModal(true)} className="mt-2 text-amber-400 hover:text-amber-300 text-sm">Add your first connection</button>
          </div>
        ) : (
                ...
                <div key={conn.id} className="rounded-xl border bg-[#171717] border-white/5 hover:border-white/10 p-5 transition-colors">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[11px] text-gray-500">{typeLabels[conn.type] || conn.type}</p>
                      </div>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-4">
                    <div>
                      ...
                      <p className="text-[12px] text-gray-300 font-mono">{config.host || "local socket"}</p>
                    </div>
                    <div>
                      <p className="text-[11px] text-gray-600">Database</p>
                      <p className="text-[12px] text-gray-300 font-mono">{config.dbName || config.companyId || "-"}</p>
                    </div>
                  </div>
                  <div className="flex items-center justify-between pt-3 border-t border-white/5">
                    <span className="text-[11px] text-gray-600">{conn.has_secret ? "🔑 Credentials stored" : "No credentials"}</span>
                    <div className="flex items-center gap-1">
                      <button onClick={() => handleTest(conn.id)} disabled={isTesting}
                        className={`px-2.5 py-1 text-[11px] rounded-md transition-colors font-medium ${isTesting ? "text-blue-400 bg-blue-500/10" : "text-gray-400 hover:text-white hover:bg-white/5"}`}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setShowAddModal(false)}>
          <div className="w-full max-w-md bg-[#171717] border border-white/10 rounded-2xl p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              ...
              <div>
                <label className="block text-[12px] font-medium text-gray-400 mb-1">Type</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[12px] font-medium text-gray-400 mb-1">Host</label>
                  <input type="text" value={formHost} onChange={(e) => setFormHost(e.target.value)} placeholder="localhost (empty for local)"
                    className="w-full px-3 py-2 bg-[#0a0a0a] border border-white/10 rounded-lg text-[13px] text-gray-200 placeholder-gray-600 focus:outline-none focus:border-amber-500/30 font-mono text-[12px]" />
                </div>
                <div>
                  <label className="block text-[12px] font-medium text-gray-400 mb-1">Port</label>
                  <input type="text" value={formPort} onChange={(e) => setFormPort(e.target.value)} placeholder="5432"
                    className="w-full px-3 py-2 bg-[#0a0a0a] border border-white/10 rounded-lg text-[13px] text-gray-200 placeholder-gray-600 focus:outline-none focus:border-amber-500/30 font-mono text-[12px]" />
                </div>
              </div>
              <div>
                <label className="block text-[12px] font-medium text-gray-400 mb-1">Database Name</label>
                <input type="text" value={formDbName} onChange={(e) => setFormDbName(e.target.value)} placeholder="buildai_demo"
                  className="w-full px-3 py-2 bg-[#0a0a0a] border border-white/10 rounded-lg text-[13px] text-gray-200 placeholder-gray-600 focus:outline-none focus:border-amber-500/30 font-mono text-[12px]" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[12px] font-medium text-gray-400 mb-1">Username</label>
                  <input type="text" value={formUsername} onChange={(e) => setFormUsername(e.target.value)} placeholder="(empty for peer auth)"
                    className="w-full px-3 py-2 bg-[#0a0a0a] border border-white/10 rounded-lg text-[13px] text-gray-200 placeholder-gray-600 focus:outline-none focus:border-amber-500/30" />
                </div>
                <div>
                  <label className="block text-[12px] font-medium text-gray-400 mb-1">Password</label>
