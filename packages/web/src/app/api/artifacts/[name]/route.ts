...
function validateAccess(agentId: string | null, actor: Awaited<ReturnType<typeof requireSignedIn>>, entityId: string) {
  ...
  if (!canAccessAgent(actor, agentId)) {
    writeAuditEvent({ actorUserId: actor.userId, action: 'artifact.access.denied', entityType: 'artifact', entityId, orgId: getAgentOrgId(agentId) || undefined, metadata: { reason: 'ORG_MISMATCH' } });
