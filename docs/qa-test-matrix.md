...
| Workspace isolation between users | `Partial` | `packages/web/__tests__/runtime-isolation-file-artifact-api.test.ts`, `packages/web/__tests__/runtime-sync.integration.test.ts`, `packages/web/__tests__/engine-config.test.ts`, `packages/web/__tests__/api-chat-ownership.test.ts`, `packages/web/__tests__/api-chat-history.test.ts`, `packages/web/__tests__/me-route-provisioning.test.ts` | File-level isolation, shared-agent chat/session namespacing, unique per-user workspace provisioning, and stale runtime/auth cleanup are covered; full multi-user concurrent execution still needs stronger end-to-end testing. |
